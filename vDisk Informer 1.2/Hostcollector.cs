                ...
                for (Int32 oci = 0; oci <= ocary.Length - 1; oci++)
                {
                    ...
                    for (Int32 propi = 0; propi <= pcary.Length - 1; propi++)
                    {
                        ...
                        if ((pc.name.Equals("parent")))
                        {
                            ...
                            XmlElement xmlhost = xmldoc.CreateElement(null, "Host", null);
                            
                            XmlNode xmlnode3 = xmldoc.SelectSingleNode("//*[@moref='" + parent + "']");
                            //XmlNode xmlnode3 = xmldoc.SelectSingleNode("//Folder[@moref='" + parent + "']");
                            XmlAttribute newAtt = xmldoc.CreateAttribute("moref");
                            newAtt.Value = moref;
                            xmlhost.Attributes.Append(newAtt);
                            xmlnode3.AppendChild(xmlhost);

                            xmlnode3 = xmldoc.SelectSingleNode("//Host[@moref='" + moref + "']");
                            XmlElement xmlhostname = xmldoc.CreateElement(null, "HostName", null);
                            xmlhostname.InnerText = hostname;
                            xmlnode3.AppendChild(xmlhostname);
                        }
                        if ((pc.name.Equals("vm")))
                        {
                            try
                            {
                                for (Int32 i = 0; i >= 0; )
                                {
                                    ...
                                    string vmmoref = ((VimyyApi.ManagedObjectReference[])(pc.val))[i].Value;
                                    XmlElement xmlvm = xmldoc.CreateElement(null, "VM", null);
                                    XmlNode xmlnode3 = xmldoc.SelectSingleNode("//Host[@moref='" + moref + "']");
                                    XmlAttribute newAtt = xmldoc.CreateAttribute("moref");
                                    newAtt.Value = vmmoref;
                                    xmlvm.Attributes.Append(newAtt);
                                    xmlnode3.AppendChild(xmlvm);
                                    i++;
                                }
                            }
                            catch
